 ...
 */
class SwipeTriggerCore : public MotionTriggerCore
{
public:
    /**
     ...
     * If minAngle > maxAngle, the range includes all values where x >= minAngle || x <= maxAngle.
     */
    SwipeTriggerCore(qreal minAngle, qreal maxAngle);
    SwipeTriggerCore(SwipeDirection direction);
...
private:
    ...
    std::optional<SwipeDirection> m_direction;

    qreal m_minAngle{};
    qreal m_maxAngle{};
    bool m_bidirectional{};

    friend class TestSwipeTriggerCore;
};
