#include "Test.h"
#include "mocks/MockSwipeTriggerCore.h"
...
class TestSwipeTriggerCore : public Test
{
    Q_OBJECT
...
private slots:
    ...
    {
        SwipeTriggerCore core(30, 60);

        SwipeTriggerUpdateEvent event;
        event.setAngle(0);
        event.setAverageAngle(45);

        QCOMPARE(core.canUpdate(event), true);
    }
    ...
    void canUpdate_currentAngleMatchesRange_averageAngleDoesNotMatchRange__returnsFalse()
    {
        ...
        QCOMPARE(core.canUpdate(event), false);
    }
    ...
    void canUpdate()
    {
        ...
        SwipeTriggerCore core(min, max);
        core.setBidirectional(bidirectional);

        SwipeTriggerUpdateEvent event;
        event.setAverageAngle(angle);

        QCOMPARE(core.canUpdate(event), returnValue);
    }

    void updateActions_bidirectional_normalAngle__updatesActionsWithPositiveDelta()
    {
        MockSwipeTriggerCore core(0, 0);
        core.setBidirectional(true);
        EXPECT_CALL(core, doUpdateActions(_, Property(&TriggerUpdateEvent::delta, Property(&Delta::unaccelerated, Eq(10))))).Times(1);

        SwipeTriggerUpdateEvent event;
        event.setAngle(0);
        event.setDelta(10);

        core.updateActions({}, event);

        QVERIFY(Mock::VerifyAndClearExpectations(&core));
    }
    ...
    {
        MockSwipeTriggerCore core(0, 270);
        core.setBidirectional(true);
        EXPECT_CALL(core, doUpdateActions(_, Property(&TriggerUpdateEvent::delta, Property(&Delta::unaccelerated, Gt(-10))))).Times(2);
    ...
    void matchesAngleRange()
    {
        ...
        QCOMPARE(SwipeTriggerCore::matchesAngleRange(angle, a, b), result);
    }
    ...
    void matchesOppositeAngleRange()
    {
        ...
        QCOMPARE(SwipeTriggerCore::matchesOppositeAngleRange(angle, a, b), result);
    }
};

}

QTEST_MAIN(InputActions::TestSwipeTriggerCore)
#include "TestSwipeTriggerCore.moc"